...
for (number, character) in alphabet.enumerated() {
...
let capitalOfTheState = ["California": "San-Francisco", "Oregon": "Oregon", "Texas": "Austin", "Moscow region": "Moscow"]
for (state, capital) in capitalOfTheState {
    if state == "Moscow region" {
